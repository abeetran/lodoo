...
class OdooAIUserBehavior(models.Model):
    ...
    behavior_type = fields.Selection([('module_visit', 'Vào Module'), ('frequent_question', 'Hỏi AI')])
...
class OdooAIChat(models.Model):
    ...
    def get_action_data(self, action_ref, domain_str=None, view_type=None):
        try:
            ...
            if ',' in action_ref:
                ...
                action = self.env['ir.actions.act_window'].sudo().browse(int(action_ref))
            ...
            if domain_str:
                try: action_dict['domain'] = json.loads(domain_str)
                except: pass
        ...
        if behavior_type == 'module_visit':
            ...
            try:
                if str(res_model).isdigit():
                    action = self.env['ir.actions.act_window'].sudo().browse(int(res_model))
                    if action.exists(): module_name, action_valid = action.name, True
                else:
                    action = self.env.ref(res_model, raise_if_not_found=False)
                    if action: module_name, action_valid = action.name, True
                if not action_valid: return 
            except Exception: return

        domain = [('user_id', '=', self.env.uid), ('behavior_type', '=', behavior_type), ('res_model', '=', res_model)]
        existing = self.env['odoo.ai.user.behavior'].search(domain, limit=1)
        if existing: existing.write({'count': existing.count + 1})
        else: self.env['odoo.ai.user.behavior'].create({'behavior_type': behavior_type, 'res_model': res_model, 'module_name': module_name or "Màn hình hệ thống"})
    ...
    def get_personalized_suggestions(self):
        ...
        for m in top_modules_raw:
            try:
                if str(m['res_model']).isdigit() and self.env['ir.actions.act_window'].sudo().browse(int(m['res_model'])).exists(): top_modules.append(m)
                elif self.env.ref(m['res_model'], raise_if_not_found=False): top_modules.append(m)
    ...
    def get_chat_history(self, chat_id):
        ...
        if record.exists():
            if record.history_json and record.history_json != '[]':
                try: return json.loads(record.history_json)
                except: pass
            msgs = []
    ...
    def send_message_from_ui(self, question=None, attachments=None, chat_id=None):
        if chat_id:
            record = self.browse(int(chat_id))
            if not record.exists(): record = self.create({'name': f'{question[:30]}...' if question else 'File Upload'})
        else: record = self.create({'name': f'{question[:30]}...' if question else 'File Upload'})
    ...
    def action_send_with_files(self, question, attachments=None):
        ...
        dynamic_actions = ""
        intent_create = any(kw in lower_q for kw in ['tạo', 'thêm', 'mới', 'create', 'new', 'add'])
        ...
        for key, xml_id in CORE_APPS.items():
            if key in lower_q.split() or f" {key} " in f" {lower_q} ":
                ...
                if not action and key in ['sale', 'bán hàng', 'đơn hàng']:
                    action = self.env.ref('sale.action_quotations', raise_if_not_found=False)
                
                if action:
                    ext_ids = action.get_external_id()
                    valid_xml_id = ext_ids.get(action.id) if ext_ids else f"{action._name},{action.id}"
        ...
        if not core_matched:
            stop_words = ['vào', 'trang', 'mở', 'module', 'app', 'ứng', 'dụng', 'đi', 'đến', 'cho', 'tôi', 'xem', 'làm', 'sao', 'muốn', 'tạo', 'thêm', 'mới', 'của', 'các', 'một']
            clean_kws = [kw for kw in lower_q.replace('?', '').replace('.', '').split() if kw not in stop_words and len(kw) >= 2]
            
            if clean_kws:
                search_term = " ".join(clean_kws)
                menus = self.env['ir.ui.menu'].sudo().search([
                    ('action', '!=', False),
                    '|', ('name', 'ilike', search_term), ('complete_name', 'ilike', search_term)
                ])
                menus = [m for m in menus if 'settings' not in str(m.action).lower() and 'config' not in str(m.action).lower()]
                menus = sorted(menus, key=lambda m: str(m.complete_name).count('/'))[:3]
                
                if not menus:
                    longest_kw = max(clean_kws, key=len)
                    menus = self.env['ir.ui.menu'].sudo().search([
                        ('action', '!=', False),
                        '|', ('name', 'ilike', longest_kw), ('complete_name', 'ilike', longest_kw)
                    ])
                    menus = [m for m in menus if 'settings' not in str(m.action).lower() and 'config' not in str(m.action).lower()]
                    menus = sorted(menus, key=lambda m: str(m.complete_name).count('/'))[:3]
                ...
                if menus:
                    ...
                    for m in menus:
                        if m.action:
                            ...
                            action_ref = ext_ids.get(m.action.id) if ext_ids else f"{m.action._name},{m.action.id}"
                            action_str = f"||OD_ACTION:{action_ref}"
                            if intent_create: action_str += "|NEW"
                            action_str += "||"
                            dynamic_actions += f"- '{m.complete_name}': {action_str}\n"

        knowledge_context = ""
        knowledges = self.env['odoo.ai.knowledge'].sudo().search([('active', '=', True)])
        for k in knowledges:
            if k.keyword:
                keywords = [kw.strip().lower() for kw in k.keyword.split(',') if kw.strip()]
        ...
        if not limit_match: limit_match = re.search(r'(top|thống kê)\s*(\d+)', lower_q)

        try:
            if limit_match or ("bán chạy" in lower_q and "sản phẩm" in lower_q):
                top_n = 5
                if limit_match:
                    top_n = int(limit_match.group(1) if limit_match.group(1).isdigit() else limit_match.group(2))
                    if top_n <= 0: top_n = 5
                ...
                if reports:
                    prod_ids = [r['product_id'][0] for r in reports if r.get('product_id')]
                    product_lines = []
                    total_revenue = 0
                    for idx, r in enumerate(reports, start=1):
                        ...
                        product_lines.append(f"{idx}. {r['product_id'][1]} - Đã xuất hóa đơn {qty} cái, doanh thu {subtotal:,.0f} VNĐ")
                        total_revenue += subtotal
                    
                    prod_str = "\n".join(product_lines)
                    domain_json = json.dumps([['product_id', 'in', prod_ids]], ensure_ascii=False)
                    ...
                    if self.history_json and self.history_json != '[]':
                        try: history = json.loads(self.history_json)
                        except: history = []
                    history.append({'role': 'user', 'content': question})
                    history.append({'role': 'ai', 'content': answer, 'suggested_action': action_id})
        ...
        except Exception as e: _logger.error(f"Lỗi truy vấn AI: {e}")
        
        erp_context = ""
        try:
            installed = self.env['ir.module.module'].sudo().search([('state', '=', 'installed'), ('application', '=', True)])
            ...
            if any(kw in lower_q for kw in ['cài', 'hoàn thiện', 'erp', 'khuyên', 'tư vấn', 'nên', 'thiếu']):
                uninstalled = self.env['ir.module.module'].sudo().search([('state', '!=', 'installed'), ('application', '=', True)], limit=15)
                uninst_names = [m.shortdesc for m in uninstalled if m.shortdesc]
...
QUY TẮC TỐI THƯỢNG:
1. TUÂN THỦ HƯỚNG DẪN CÔNG TY: Nếu prompt có [HƯỚNG DẪN ĐẶC THÙ CỦA DOANH NGHIỆP], bạn BẮT BUỘC phải dùng toàn bộ thông tin đó để trả lời khách hàng. TUYỆT ĐỐI KHÔNG TỪ CHỐI câu hỏi (ví dụ: không được nói "tôi chỉ hỗ trợ Odoo...").
2. NẾU CÓ [SỐ LIỆU THẬT]: TRÌNH BÀY CHÍNH XÁC. KHÔNG TỰ BỊA SỐ.
3. LUÔN đính kèm mã action ||OD_ACTION:xxx|| ở cuối câu trả lời nếu cần điều hướng.
4. Cấm dùng thẻ Markdown tạo link html.] \n\n""")        
        ...
        full_prompt_for_ai = system_note + dynamic_actions + knowledge_context + erp_context
        ...
        try:
            res = requests.post(mcp_url, json=payload, headers=headers, timeout=60).json()
            ans = res.get('reply', '')
            action_id = False
            ...
            if attachments: user_msg["content"] += f"\n[Đã đính kèm {len(attachments)} tệp]"
            history.append(user_msg)
            
            ai_msg = {"role": "ai", "content": ans}
            if action_id: ai_msg["suggested_action"] = action_id
        ...
        except Exception as e: return {'chat_id': self.id, 'answer': f"Lỗi Server: {str(e)}"}